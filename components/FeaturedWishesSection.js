...
const featuredWishes = [
  ...
  { name: "Playground for Kids", raised: 450, goal: 600 },
  { name: "Books for Library", raised: 200, goal: 300 },
  { name: "Water Well Project", raised: 800, goal: 1000 },
  { name: "Scholarship Fund", raised: 1000, goal: 1500 },
];

export default function FeaturedWishesSection() {
  useEffect(() => {
    AOS.init({ duration: 1000, once: true, offset: 50 }); // smaller offset
  }, []);

  return (
    <section className="py-16 bg-white dark:bg-slate-900 overflow-x-hidden">
        ...
        <div className="grid sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
          {featuredWishes.map((wish, i) => (
            <div
              ...
              className="p-6 bg-slate-50 dark:bg-slate-800 rounded-2xl shadow-xl border-4 border-slate-300 dark:border-slate-700 max-w-full box-border transition-transform hover:scale-105"
            >
